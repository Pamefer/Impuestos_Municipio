				...
				<?php 		
						...
						if ($id_usuario==''){
							...
							echo "<div class='panel-body'>
								    <p>Detalle de deudas de <strong>".$id_usuario[1]." ".$id_usuario[2]."</strong> con la cédula <b>".$_GET['cedula']. ":</p>
								  	<a  href='$url_site/public_html/cancelas.php?cedula=".$_GET['cedula']."'><strong>Ver Deudas Canceladas</strong></a>
								  </div>";						
							...
							$miconexion->consulta("SELECT t_s.servicio as DEUDA, s_u.fecha, CONCAT ('$', s_u.valorpagar) as Valor, CONCAT ('Consumo= ', s_u.consumo) as Detalle  FROM servicios_usuarios as s_u  
								...
								WHERE id_usuario='$id_usuario[0]' and estado='PENDIENTE'
								order by fecha");
							...
							//PARA FOTOMULTAS:							  
							$miconexion->consulta("SELECT CONCAT (fecha, ' ', hora) as fecha,  CONCAT ('$', valorpagar) as Valor, CONCAT ('Velocidad= ', velocidad, ' Lugar=', lugar) as Detalle  FROM fotomultas_usuarios WHERE USUARIOS_id_usuario='$id_usuario[0]' and estado='PENDIENTE' order by fecha");
