 ...
 <!-- Bootstrap core CSS -->
  ...
  <section id="container" >
    ...
    <section class="wrapper site-min-height">
            <h3><i class="fa fa-angle-right"></i> Deudas canceladas de <?php echo $_GET['nombres'] ?> con cedula <?php echo $_GET['cedula'] ?></h3>
              <section id="po">
            <div class="row mt">
              <div class="col-lg-12">
                <div class="row"> 
                   ...
                   <?php 
                      ...
                      $miconexion->consulta("SELECT t_s.servicio as DEUDA, s_u.fecha, CONCAT ('$', s_u.valorpagar) as Valor, CONCAT ('Consumo= ', s_u.consumo) as Detalle, s_u.id_ser_usuar as pagar FROM servicios_usuarios as s_u  
                        ...
                        WHERE id_usuario='$_GET[id]' and estado='PAGADO'
                        order by fecha");
                      
                      //ENCABEZADO PARA TABLA:
                      $miconexion->lista_encabezado_pagos();

                      $sin_deudas=$miconexion->lista_servicios_pagos();
                      $vacio=0;                      
                      ...
                      //PARA FOTOMULTAS:                
                      $miconexion->consulta("SELECT CONCAT (fecha, ' ', hora) as fecha,  CONCAT ('$', valorpagar) as Valor, CONCAT ('Velocidad= ', velocidad, ' Lugar=', lugar) as Detalle, id_foto_usuar as pagar  FROM fotomultas_usuarios WHERE USUARIOS_id_usuario='$_GET[id]' and estado='PAGADO' order by fecha");
                      
                      $sin_deudas=$miconexion->lista_fotomulta_pagos();
                      ...
                      if ($vacio==2) {
                        echo "<h4>El Usuario, no posee deudas canceladas.</h4>";
                      }
